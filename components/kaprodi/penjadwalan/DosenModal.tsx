				...
				<div className="w-full flex flex-col gap-3">
					<div className="w-full flex items-center justify-between">
						<div className="w-full flex gap-4">
							<button className={`py-2 px-4 rounded ${mode == 1 ? 'bg-blue-500 text-white' : 'bg-gray-300'}`} onClick={() => { setMode(1)}}>
								Preferensi
							</button>
							<button className={`py-2 px-4 rounded ${mode == 2 ? 'bg-blue-500 text-white' : 'bg-gray-300'}`} onClick={() => { setMode(2)}}>
								Lainnya
							</button>
						</div>
					...
					<div className="max-h-96 overflow-hidden">
						...
						{filteredDosen && filteredDosen.length > 0 ? (
						filteredDosen.map((dosen, index) => (
							<div key={index} className="flex gap-5 lowercase justify-between items-center text-nowrap">
								...
								<button className="py-1 px-3 bg-blue-500 text-white rounded hover:bg-blue-700" onClick={() => {result(dosen.id); setSearchQuery(''); }}>
									Pilih
								</button>
							</div>
						))
							...
							<div className="text-center text-xl text-gray-500">
								Tidak ada dosen yang memilih mata kuliah ini
							</div>
						)}
						</div>
					</div>
