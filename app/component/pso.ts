...
function initializeParticles(
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        callbacks.sendProgress({
            stage: `Inisialisasi partikel ke ${i + 1}/${numParticles}`
        });
    }
...
function evaluateFitness(position: [number, number][], ctx: ScheduleContext): number {
    ...
    // 1. Check Time Preferences (not used in original code)
    ...
    // 4. Check Day Preferences (not used in original code)
    ...
    return -fitness; // Return negative fitness if higher score is better
}

export function PARTICLE_SWARM_OPTIMIZATION(
    schedules: Schedules[],
    para: Parameter,
    callbacks: { sendProgress: (data: any) => void }
): OptimizedSchedule[] {
    ...
    for (let iteration = 0; iteration < para.num_iteration; iteration++) {
        for (let i = 0; i < particles.length; i++) {
            callbacks.sendProgress({
                stage: `Iterasi ke ${iteration + 1}/${para.num_iteration} - Mengevaluasi dan memperbarui partikel ke ${i + 1}`,
...
function buildScheduleStructures(
    ...
    groups: { [key: number]: { [key: number]: { [key: number]: { [key: number]: { [key: string]: ScheduleInfo[] } } } } }
): void {
...
function checkTimePreference(position: [number, number][], ctx: ScheduleContext): [number, number] {
    ...
    position.forEach((pos, i) => {
        ...
        if ((ctx.TimePreferences[i] === "Pagi" && timeGroup !== 0) ||
            (ctx.TimePreferences[i] === "Malam" && timeGroup !== 1)) {
            penalty += 5000;
            conflicts++;
        }
    });
